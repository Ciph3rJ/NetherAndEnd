...
public class ShulkerFalchionItem extends Item {
    ...
    public InteractionResult use(Level level, Player player, InteractionHand interactionHand) {
        if (player.getInventory().contains(NetherAndEndItems.SHULKER_PEARL.getDefaultInstance())) {
            if (player.getLastHurtMob() != null) {
                ...
                for (int slot = 0; slot < player.getInventory().getContainerSize(); slot++) {
                    if (player.getInventory().getItem(slot).is(NetherAndEndItems.SHULKER_PEARL)) {
