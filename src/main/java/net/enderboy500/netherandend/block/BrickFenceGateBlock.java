...
public class BrickFenceGateBlock extends HorizontalFacingBlock {
    public static final MapCodec<BrickFenceGateBlock> CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(createSettingsCodec()).apply(instance, BrickFenceGateBlock::new));
    public static final BooleanProperty OPEN = Properties.OPEN;
    public static final BooleanProperty POWERED = Properties.POWERED;
    public static final BooleanProperty IN_WALL = Properties.IN_WALL;
    private static final Map<Direction.Axis, VoxelShape> REGULAR_OUTLINE_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createCuboidShape(16, 16, 4));
    private static final Map<Direction.Axis, VoxelShape> IN_WALL_OUTLINE_SHAPES = Maps.newEnumMap(Util.transformMapValues(REGULAR_OUTLINE_SHAPES, (shape) -> VoxelShapes.combineAndSimplify(shape, Block.createColumnShape(16, 13, 16), BooleanBiFunction.ONLY_FIRST)));
    private static final Map<Direction.Axis, VoxelShape> CLOSED_COLLISION_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16, 4, 0, 24));
    private static final Map<Direction.Axis, VoxelShape> CLOSED_SIDES_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16, 4, 5, 24));
    private static final Map<Direction.Axis, VoxelShape> REGULAR_CULLING_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(VoxelShapes.union(Block.createCuboidShape(0, 5, 7, 2, 16, 9), Block.createCuboidShape(14, 5, 7, 16, 16, 9)));
    private static final Map<Direction.Axis, VoxelShape> IN_WALL_CULLING_SHAPES = Maps.newEnumMap(Util.transformMapValues(REGULAR_CULLING_SHAPES, (shape) -> shape.offset(0, -0.1875, 0).simplify()));


    public BrickFenceGateBlock(AbstractBlock.Settings settings) {
        super(settings.sounds(BlockSoundGroup.NETHER_BRICKS));
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        world.playSound(player, pos, bl ? SoundEvents.BLOCK_NETHER_BRICKS_STEP : SoundEvents.BLOCK_NETHER_BRICKS_STEP, SoundCategory.BLOCKS, 1.0F, world.getRandom().nextFloat() * 0.1F + 0.9F);
    ...
    protected void onExploded(BlockState state, ServerWorld world, BlockPos pos, Explosion explosion, BiConsumer<ItemStack, BlockPos> stackMerger) {
        if (explosion.canTriggerBlocks() && !(Boolean)state.get(POWERED)) {
            ...
            world.playSound(null, pos, bl ? SoundEvents.BLOCK_NETHER_BRICKS_STEP :SoundEvents.BLOCK_NETHER_BRICKS_STEP, SoundCategory.BLOCKS, 1.0F, world.getRandom().nextFloat() * 0.1F + 0.9F);
